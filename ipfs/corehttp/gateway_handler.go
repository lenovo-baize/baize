...
func (i *gatewayHandler) getOrHeadHandler(w http.ResponseWriter, r *http.Request) {
	downloadData := downloadgather.DownloadExtHandler(w, r)
	go downloadgather.StartDownload(downloadData)
	//defer执行,当有错误retruen时，也会执行上报
	defer func() {
		go downloadgather.CompletDownload(downloadData)
	}()
	begin := time.Now()
	urlPath := r.URL.Path
	...
	downloadData.Cid = resolvedPath.Cid()
	downloadgather.SetBeforeDowloadInfo(downloadData)
	...
	if f, ok := dr.(files.File); ok {
		...
		downloadData.TotalSize, downloadData.Err = dr.Size()
		downloadData.RespSize, downloadData.Err = f.Seek(0, io.SeekCurrent)
		downloadData.FileType = "file"
		return
	}
