...
//IpfsMain 白泽调用的入口方法，修改了原ipfs的mainRet方法，返回类型该成了error,去掉了main方法
func IpfsMain(pctx context.Context) error {
	rand.Seed(time.Now().UnixNano())
	ctx := logging.ContextWithLoggable(pctx, loggables.Uuid("session"))
	var err error
	...
	return cli.Run(ctx, Root, os.Args, os.Stdin, os.Stdout, os.Stderr, buildEnv, makeExecutor)
