...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) (errrs error) {
	defer func() {
		if r := recover(); r != nil {
			...
			baizectx.GetContext().DaemonErr = errrs.Error()
		}
	}()
	err := runDaemon(req, re, env)
	if nil != err {
		baizectx.GetContext().DaemonErr = err.Error()
	}
	return err
}
func runDaemon(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) error {
	...
	if initialize {
		...
		if !fsrepo.IsInitialized(cfg) {
			...
			cfgmgr.SyncIpfsConfig()
		}
	}

	if !fsrepo.IsInitialized(cctx.ConfigRoot) {
		return errors.New("error init ipfs")
	}
